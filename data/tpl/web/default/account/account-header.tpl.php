...
<div class="media media-wechat-setting">
	...
	<div class="media-right media-middle">
		<a href="<?php  echo url('account/manage/delete', array('uniacid' => $_GPC['uniacid'], 'acid' => $_GPC['acid'], 'type' => ACCOUNT_TYPE))?>" class="btn btn-primary" onclick="return confirm('确认放入回收站吗？')">停  用</a>
	</div>
	<?php  } ?>
</div>
...
<div class="btn-group we7-btn-group wechat-edit-group">
	...
	<?php  } ?>
	<?php  if(ACCOUNT_TYPE == ACCOUNT_TYPE_APP_NORMAL) { ?>
	<a href="<?php  echo url('account/post-version/display', array('uniacid' => $_GPC['uniacid'], 'acid' => $_GPC['acid'], 'account_type' => ACCOUNT_TYPE_APP_NORMAL))?>" class="btn btn-default <?php  if($action == 'post-version' && $do == 'display') { ?> active<?php  } ?>">版本管理</a>
	<?php  } ?>
</div>