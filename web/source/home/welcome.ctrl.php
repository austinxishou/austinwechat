...
if ($do == 'platform') {
	...
	$_W['page']['title'] = '公众号平台相关数据';
	$yesterday = date('Ymd', strtotime('-1 days'));
	$yesterday_stat = pdo_get('stat_fans', array('date' => $yesterday, 'uniacid' => $_W['uniacid']));
	$yesterday_stat['new'] = intval($yesterday_stat['new']);  //新增
	$yesterday_stat['cancel'] = intval($yesterday_stat['cancel']); //取消关注
	$yesterday_stat['cumulate'] = intval($yesterday_stat['cumulate']); //净增关注
